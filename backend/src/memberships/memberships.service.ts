...
type MembershipPlanWithFeatures = MembershipPlan & {
    membershipPlanFeatures?: Array<MembershipPlanFeature & { feature: Feature | null }>;
};

type SubscriptionWithRelations = Subscription & {
    membershipPlan?: MembershipPlanWithFeatures | null;
...
export class MembershipsService {
    ...
    async createPlan(
        ...
        await this.notificationsService.notifyIfEnabled('newPaymentNotification', {
            role: UserRole.ADMIN,
    ...
    async assignMembership(
        ...
        const fullName = memberUser
            ...
            : 'Member';
        if (member.userId) {
            await this.notificationsService.notifyRoleAndUserIfEnabled(
            ...
            await this.notificationsService.notifyIfEnabled('newMembershipNotification', {
                role: UserRole.ADMIN,
                title: 'Membership assigned',
                message: `${fullName} was assigned a membership.`,
                type: 'info',
    ...
    async generateInvoicePdf(
        membershipId: string,
        currentUser: Pick<CurrentUserPayload, 'userId' | 'role'>,
    ...
    private toPlanResponseDto(plan: MembershipPlanWithFeatures): MembershipPlanResponseDto {
        return {
            id: plan.id,
            name: plan.name,
            ...
            planFeatures: Array.isArray(plan.membershipPlanFeatures)
                ? plan.membershipPlanFeatures.map((planFeature) => ({
    ...
    private toMembershipResponseDto(membership: SubscriptionWithRelations): MembershipResponseDto {
