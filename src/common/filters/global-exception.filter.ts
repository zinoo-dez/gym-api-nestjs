    ...
    private buildErrorResponse(
        ...
        if (exception instanceof HttpException) {
            ...
            if (typeof exceptionResponse === 'string') {
                ...
                if (exceptionResponse.includes('JSON at position') ||
                    exceptionResponse.includes('property name') ||
                    exceptionResponse.includes('Unexpected token')) {
                    message = 'Invalid JSON format in request body. Please ensure all property names are enclosed in double quotes and the JSON syntax is correct.';
                } else {
                ...
                let originalMessage = responseObj.message || exception.message;
    ...
    private logError(exception: unknown, request: Request): void {
        ...
        if (exception instanceof HttpException) {
            ...
            this.logger.warn(
                ...
                JSON.stringify({
                    ...errorContext,
                    rawBody: typeof request.body === 'string' ? request.body : 'Not available',
                }, null, 2),
            );
