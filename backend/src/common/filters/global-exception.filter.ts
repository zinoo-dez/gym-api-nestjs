  ...
  private buildErrorResponse(
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof exceptionResponse === 'string') {
        ...
        if (
          exceptionResponse.includes('JSON at position') ||
          exceptionResponse.includes('property name') ||
          exceptionResponse.includes('Unexpected token')
        ) {
          message =
            'Invalid JSON format in request body. Please ensure all property names are enclosed in double quotes and the JSON syntax is correct.';
        } else {
        ...
        const originalMessage = responseObj.message || exception.message;

        // Check if this is a JSON parsing error
        if (
          typeof originalMessage === 'string' &&
          (originalMessage.includes('JSON at position') ||
            originalMessage.includes('property name') ||
  ...
  private logError(exception: unknown, request: Request): void {
    ...
    if (exception instanceof HttpException) {
      ...
      if (status >= 400 && status < 500) {
        ...
        if (this.fileLogger) {
          this.fileLogger.logHttpError(exception, {
            method,
            url,
            body: sanitizedBody,
            query: sanitizedQuery,
        ...
        JSON.stringify(
          {
            ...errorContext,
            rawBody:
              typeof request.body === 'string' ? request.body : 'Not available',
          },
          null,
          2,
        ),
      ...
      if (this.fileLogger) {
        this.fileLogger.error(
          ...
          undefined,
          'UnknownException',
          { exception, context: errorContext },
        );
      }
    }
  }
