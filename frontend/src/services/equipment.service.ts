...
export const equipmentService = {
    async listEquipmentPaginated(
        ...
        const response = await api.get<ApiEnvelope<ApiPaginatedResponse<EquipmentRecordApi>>>(
            '/equipment/paginated',
            {
    ...
    async getEquipmentById(id: string): Promise<EquipmentRecord> {
        const response = await api.get<ApiEnvelope<EquipmentRecordApi>>(`/equipment/${id}`);
    ...
    async createEquipment(values: EquipmentFormValues): Promise<EquipmentRecord> {
        const response = await api.post<ApiEnvelope<EquipmentRecordApi>>(
            '/equipment',
    ...
    async updateEquipment(
        ...
        const response = await api.patch<ApiEnvelope<EquipmentRecordApi>>(
            `/equipment/${id}`,
            toUpsertPayload(values),
        );
        return normalizeEquipmentRecord(response.data.data);
