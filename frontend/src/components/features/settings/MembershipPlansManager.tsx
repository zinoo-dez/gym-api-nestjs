...
export function MembershipPlansManager({
    ...
    const planColumns: DataTableColumn<MembershipPlan>[] = [
        { id: "name", label: "Name", render: (row) => <span className="font-medium">{row.name}</span> },
        { id: "price", label: "Price", render: (row) => <span className="text-muted-foreground">{row.price.toFixed(2)}</span> },
        { id: "duration", label: "Duration", render: (row) => <span className="text-muted-foreground">{row.duration === "YEARLY" ? "Yearly" : "Monthly"}</span> },
        {
            id: "features",
            label: "Features",
            render: (row) => (
                <div className="flex flex-wrap gap-1">
                    {row.features.map((feature, i) => (
                        <span key={`${row.id}-feature-${i}`} className="inline-flex items-center rounded-full bg-secondary px-2 py-0.5 text-xs text-secondary-foreground">
                            {feature}
                        </span>
                    ))}
                </div>
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex justify-end gap-2">
                    <Button type="button" size="sm" variant="outline" onClick={() => startEditing(row)} disabled={isSubmitting}>
                        <Pencil className="size-4" /> Edit
                    </Button>
                    <Button type="button" size="sm" variant="destructive" onClick={() => void handleDelete(row.id)} disabled={activeDeleteId === row.id || isSubmitting}>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <DataTable<MembershipPlan>
                            ...
                            emptyDescription="Add your first tier above."
                            minWidth="620px"
                        />
                    </div>
                </CardContent>
