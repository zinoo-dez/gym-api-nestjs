...
const MarketingAnalytics = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <div className="space-y-2">
          ...
          <Select value={selectedCampaignId} onValueChange={setSelectedCampaignId}>
            <SelectTrigger className="h-12 max-w-xl rounded-xl border-border bg-muted/50 focus:ring-blue-600 font-medium">
      ...
      {!selectedCampaignId ? (
        <section className="py-20 text-center rounded-2xl border-2 border-dashed border-border bg-card/50">
          <TrendingUp className="h-12 w-12 mb-3 mx-auto text-muted-foreground/70" />
          <p className="font-medium text-muted-foreground">Select a campaign to initialize analytics workbench.</p>
        </section>
      ) : !analytics ? (
        <section className="py-20 text-center rounded-2xl border-2 border-dashed border-border bg-card/50">
          <RefreshCcw className="h-12 w-12 mb-3 mx-auto text-blue-100 animate-spin" />
          <p className="font-medium text-muted-foreground italic">Assembling data points for {selectedCampaignName}...</p>
        </section>
      ) : (
        <>
          ...
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              <div className="space-y-4 p-5 rounded-2xl bg-muted/50 border border-border">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-center">Open Effectiveness</p>
                <div className="flex flex-col items-center">
                  <span className="text-4xl font-bold font-mono tracking-tighter text-blue-600">{analytics.openRate}%</span>
                  <div className="w-full h-1.5 bg-muted/80 rounded-full mt-4 overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${analytics.openRate}%` }} />
                  </div>
                  <p className="text-[10px] text-muted-foreground mt-2 italic font-medium">Recipient view frequency</p>
                </div>
              </div>

              <div className="space-y-4 p-5 rounded-2xl bg-muted/50 border border-border">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-center">Click-Through Efficiency</p>
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2 italic font-medium">Link interaction frequency</p>
                </div>
              </div>

              <div className="space-y-4 p-5 rounded-2xl bg-muted/50 border border-border">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-center">Reliability Score</p>
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2 italic font-medium">Infrastructure success rate</p>
                </div>
              </div>
            </div>
          </section>
